...
// 1) Включаем merge дефолтов только в dev при явном флаге
export const mergeDefaultTypesAndFieldsForDebugging = isDebugging => {
  const isDev = process.env.NODE_ENV === 'development';
  return Boolean(isDebugging && isDev);
};

// 2) Приводим hosted listingTypes в форму, с которой работает клиент
export const restructureListingTypes = listingTypes => {
  if (!Array.isArray(listingTypes)) return [];
  return listingTypes
    .map(t => {
      const { id, label, transactionProcess, unitType, ...rest } = t || {};
...
export const union = (arr1 = [], arr2 = [], key = 'key') => {
  const all = [...arr1, ...arr2];
...
const mergeLocalizations = (hostedLocalization, defaultLocalization) => {
  // ✅ Приоритет локальных настроек (например, выбранный язык в configDefault.js)
  const locale =
    defaultLocalization?.locale ||
    hostedLocalization?.locale ||
    'en';
...
// Function moved from configListing.js to avoid circular dependency
export const mergeListingConfig = (hostedConfig, defaultConfigs, categoriesInUse) => {
  // Что пришло из Console
  ...
  // Локальные дефолты - берём из defaultConfigs.listing если есть
  const {
