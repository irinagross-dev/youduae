...
export const TransactionPageComponent = props => {
  ...
  const showBookingLocation =
    ...
    // --- Закрытие задания (кнопка ТОЛЬКО для Provider после accept-offer)
const lastTransition = transaction?.attributes?.lastTransition;

// владелец ли сделки (см. isOwnSale чуть выше в файле)
const isProviderOwnSale = isOwnSale;

// ✅ ИЗМЕНЕНО: Теперь ТОЛЬКО Provider (заказчик) может закрывать задание
// Provider закрывает задание после того, как работа выполнена
const canComplete =
