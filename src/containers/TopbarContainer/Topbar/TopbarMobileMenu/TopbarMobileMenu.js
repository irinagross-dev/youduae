...
const TopbarMobileMenu = props => {
  ...
  // Фильтруем customLinks - убираем все ссылки на авторизацию
  ...
  const extraLinks = filteredCustomLinks.map((linkConfig, index) => {
    return (
      <CustomLinkComponent
  ...
  if (!isAuthenticated) {
    const createListingLink = showCreateListingsLink ? (
    ...
    const authLink = (
      <NamedLink name="LoginPage" className={css.loginLink}>
        <FormattedMessage id="TopbarMobileMenu.authLink" />
      </NamedLink>
    );
