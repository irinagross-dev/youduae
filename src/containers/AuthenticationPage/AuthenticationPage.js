...
export const AuthenticationPageComponent = props => {
  ...
  // Check for guest listing creation intent FIRST (before email verification)
  const searchParams = new URLSearchParams(location.search);
  const urlIntent = searchParams.get('intent');
  const localStorageIntent = typeof window !== 'undefined' ? localStorage.getItem('pendingListingIntent') : null;
  const hasCreateListingIntent = urlIntent === 'createListing' || localStorageIntent === 'createListing';

  // We only want to show the email verification dialog in the signup
  // tab if the user isn't being redirected somewhere else
  // (i.e. `from` is present or `hasCreateListingIntent`). 
  // We must also check the `emailVerified` flag only when the current user is fully loaded.
  const showEmailVerification = !isLogin && currentUserLoaded && !user.attributes.emailVerified && !hasCreateListingIntent && !from;
  ...
  if (shouldRedirectToCreateListing) {
