...
export function requestImageUpload(actionPayload, listingImageConfig) {
  return (dispatch, getState, sdk) => {
    ...
    const file = actionPayload.file;
    
    console.log('🔍 requestImageUpload called with:', {
      id,
      hasFile: !!file,
    ...
    if (!(file instanceof File)) {
      console.error('❌ actionPayload.file is NOT a File object!', file);
      return Promise.reject(new Error('Invalid file: expected File object'));
    }
    ...
    console.log('📤 SDK upload starting with File:', {
      name: file.name,
      size: file.size,
      type: file.type
    });
    ...
    return sdk.images
      ...
      .then(resp => {
        console.log('✅ SDK upload success:', resp);
        const img = resp.data.data;
        console.log('🖼️ Extracted image data:', {
          imageId: img.id,
          imageIdType: typeof img.id,
          imageIdValue: img.id?.uuid || img.id,
          hasId: !!img.id,
        ...
        const successData = { data: { ...img, id, imageId: img.id, file: actionPayload.file } };
        dispatch(uploadImageSuccess(successData));
        
        // IMPORTANT: Return the data, not the dispatch result!
        console.log('✅ Returning image data from requestImageUpload:', {
          id,
          imageId: img.id,
          imageIdUuid: img.id?.uuid || img.id
        });
        return successData;
      })
      .catch(e => {
        ...
        // IMPORTANT: Return null for failed uploads (not dispatch result)
        return null;
      });
  };
}
