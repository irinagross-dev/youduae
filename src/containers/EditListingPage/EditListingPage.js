...
import { hasPermissionToPostListings, isUserAuthorized, getCurrentUserTypeRoles } from '../../util/userHelpers';
...
export const EditListingPageComponent = props => {
  ...
  // CRITICAL BUSINESS LOGIC: Check if user is a customer (executor)
  ...
  // CRITICAL FIX: If URL has a temporary ID (e.g., "new-1234567890"), 
  // ignore submittedListingId to allow creating a new listing
  const isTemporaryId = id && id.startsWith('new-');
  const listingId = isTemporaryId ? null : (page.submittedListingId || (id ? new UUID(id) : null));
  ...
  if (!isUserAuthorized(currentUser)) {
    ...
    return (
      <Page title={intl.formatMessage(loadingPageMsg)} scrollingDisabled={true}>
        <TopbarContainer
          mobileRootClassName={css.mobileTopbar}
          desktopClassName={css.desktopTopbar}
          mobileClassName={css.mobileTopbar}
