...
const pathParamsToPrevTab = (params, tab, marketplaceTabs) => {
  const currentTabIndex = marketplaceTabs.findIndex(s => s === tab);
  const prevTab = currentTabIndex > 0 ? marketplaceTabs[currentTabIndex - 1] : marketplaceTabs[0];
...
const EditListingWizardTab = props => {
  ...
  const panelProps = tab => {
    ...
    const handleGoBack = () => {
      ...
      const to = createResourceLocatorString('EditListingPage', routeConfiguration, prevPathParams, {});
      history.push(to);
    };
    ...
    return {
      ...
      onGoBack: !isFirstTab ? handleGoBack : null,
      isFirstTab,
    };
  };
